...
class Molecule:
        ...
        def __init__(self):
                ...
                self.Eref={'CH4':-22.100,'H2O':-12.247112,'H2':-6.8964381,'NH3':-17.523142} #DFT energies of the reference species (ZPE corrected) in eV
                self.Eslab=-100.62901165 #DFT energy of the slab in eV
                self.dHrxnatct={'H2-2H':432.068, 'O2-2O':493.688, 'N2-2N':941.157}
                self.Erefbeeslab=data['Pt'].to_numpy()*self.scale_BEEF
                self.Erefbee={'CH4':data['CH4'].to_numpy()*self.scale_BEEF,'H2O':data['H2O'].to_numpy()*self.scale_BEEF,'H2':data['H2'].to_numpy()*self.scale_BEEF, 'NH3':data['NH3'].to_numpy()*self.scale_BEEF}
...
def compute_thermo_bee(molecule):
    ...
    for i in range(molecule.N_BEE):
        ...
        if molecule.name=='H_ads':
            ...
            molecule.energy_gas_bee[i]=(molecule.DFT_energy_gas-molecule.gas_BEE_energies[i]*molecule.scale_BEEF+molecule.ZPE_energy_gas+molecule.dHrxnatct['N2-2N']/molecule.eV_to_kJpermole)/2
        else:
