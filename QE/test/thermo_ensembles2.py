...
for k,l in enumerate(label[61:]):
     ...
     title="$\mathbf{" + str(species['species'][k+offset]) + "}$"
     axs[col,row].set_title(title)

axs[-2,-2].set_visible(False) # to remove last plot
axs[-2,-1].set_visible(False) # to remove last plot
axs[-1,0].set_visible(False) # to remove last plot
axs[-1,1].set_visible(False) # to remove last plot
axs[-1,2].set_visible(False) # to remove last plot
axs[-1,3].set_visible(False) # to remove last plot
axs[-1,4].set_visible(False) # to remove last plot
axs[-1,5].set_visible(False) # to remove last plot

plt.savefig('uq_thermo_library2.pdf', transparent=False, bbox_inches='tight')